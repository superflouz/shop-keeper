using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class BasicAttackController : Controller
{
    public float range;
    protected Attack attack;

    // Start is called before the first frame update
    new void Awake()
    {
        base.Awake();
        attack = GetComponent<Attack>();
    }

    // Update is called once per frame
    void Update()
    {
        // Check if can attack
        if (entity.State == EntityState.Idle)
        {
            Vector2 positionTarget = (Vector2)entity.transform.position;
            positionTarget.y = 0;
            positionTarget += Vector2.right * entity.slotCount * entity.transform.localScale.x * (float)range;

            // Maths for overlap
            Vector2 a = entity.transform.position;
            Vector2 b = positionTarget + Vector2.up * (0.2f + range - 1 + entity.transform.position.y);

            Entity target = null;

            // Overlap to find ennemy
            Collider2D[] hits = Physics2D.OverlapAreaAll(a, b, 1 << LayerMask.NameToLayer("Entities"));
            foreach (Collider2D hit in hits)
            {
                Entity hitEntity = hit.GetComponent<Entity>();
                // Check the first enemy in front of the entity
                if (hitEntity != null && hitEntity.Party != null && hitEntity.Party.faction != entity.Party.faction)
                {
                    if (target == null || Vector2.Distance(entity.transform.position, target.transform.position) < Vector2.Distance(entity.transform.position, hitEntity.transform.position))
                        target = hitEntity;
                }
            }

            // Trigger attack if a target is found
            if (target != null)
                attack.PrepareAttack(target);
        }
    }
}
